...
public class SimplificadoController {
    ...
    public void clicarEnviar()throws Exception{
        if(nome.getText().isEmpty()||cpf.getText().isEmpty()||contrOficial.getText().isEmpty()||totalRendimentos.getText().isEmpty()){
            JOptionPane.showMessageDialog(null,"Preencha o formulario corretamente.");

        }else{

            PessoaFisica p = new PessoaFisica(nome.getText(),cpf.getText(),new BigDecimal(contrOficial.getText()),new BigDecimal(totalRendimentos.getText()));
            CalculaImposto calcula = new CalculaImposto(p);
            PessoaImposto pessoa = new PessoaImposto(p.getNome(),p.getCPF(),p.getIdade(),p.getNumeroDependentes(),p.getContribuicaoPrevidenciariaOficial(),p.getTotalRendimentos(),calcula.calculoSimplificado());
